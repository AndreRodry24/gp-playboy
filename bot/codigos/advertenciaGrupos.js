...
async function handleMessage(c, message) {
    ...
    if (msg && msg.imageMessage) {
        ...
        if (imageCaption && imageCaption.includes('#adv') && isAdmin) {
            ...
            await sendMessage(c, from, `@${sender.split('@')[0]}, você *NÃO tem permissão* para executar esse comando 🚫👨🏻‍✈️ *Ele é EXCLUSIVO dos administradores* do grupo 👑🐰 *Ƥˡ𝒶Ƴ乃𝔬у* ♕🎀`, sender);
        }
    }
...
async function tratarAdvertencia(c, groupId, userId) {
    ...
    if (advertencias[userId] >= 3) {
        ...
        await sendMessage(c, groupId, `@${userId.split('@')[0]}, *você foi removido do grupo ❌ devido a três advertências anteriores. \n \n Por favor, revise as regras para evitar futuras penalizações.*`, userId);
        delete advertencias[userId]; // Reiniciar o contador
    } else {
        await sendMessage(c, groupId, `@${userId.split('@')[0]}, você recebeu uma *ADVERTÊNCIA* ${advertencias[userId]}/3 ⚠️ por violar as regras do grupo do 👑🐰 *Ƥˡ𝒶Ƴ乃𝔬у* ♕🎀 \n \n Por favor, leia as regras antes de postar. *Lembre-se:* ao atingir *três advertências*, você será removido do grupo.❌`, userId);
    }
}
